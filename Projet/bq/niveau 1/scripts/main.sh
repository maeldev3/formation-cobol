...
show_menu() {
    ...
    echo "│ 3. Transactions                     │"
    echo "│ 4. Gestion Credits                  │"
    echo "│ 5. Batch Mensuel                    │"
    echo "│ 6. Rapports                         │"
    echo "│ 7. Statistiques                     │"
    echo "│ 0. Quitter                          │"
    echo "└─────────────────────────────────────┘"
...
# Dans le case du menu principal
while true; do
    show_menu
    read choix
    case $choix in
...
menu_credits() {
    while true; do
        clear
        echo "--- GESTION CREDITS ---"
        echo "1. Lister credits"
        echo "2. Ajouter credit"
        echo "3. Rechercher credit par compte"
        echo "0. Retour"
        echo -n "Choix : "
        read sub
        case $sub in
            1) bin/LISTER_CREDITS ;;
            2)
                read -p "COMPTE: " num
                read -p "MONTANT: " mt
                read -p "TAUX (%): " tx
                read -p "DUREE (mois): " duree
                echo -e "$num\n$mt\n$tx\n$duree" | bin/AJOUT_CREDIT
                ;;
            3)
                read -p "COMPTE: " num
                echo "$num" | bin/RECHERCHE_CREDIT
                ;;
            0) break ;;
        esac
        read -p "Appuyez sur Entrée..."
